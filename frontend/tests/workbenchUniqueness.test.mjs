...
assert.match(structureSource, /ensure_system_unique_value\(db, TWVersion, system\.id, "version_no", data\.version_no, "版本号"\)/)
...
assert.match(formSource, /result\.message/)
assert.match(structureSource, /"message": f"版本号低于当前系统已有最高版本/)
...
assert.match(testCasePageSource, /<WorkbenchPageHeader title="用例管理"\s*\/>/)
