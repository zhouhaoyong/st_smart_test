...
const requestDetail = read('../src/components/ExecutionRequestDetail.vue')
...
assert.match(debugPanel, /<ExecutionRequestDetail[\s\S]*:request-url="selectedStep\.request\?\.url"/, '步骤详情应复用统一请求信息组件')
assert.match(requestDetail, /<div class="request-meta">[\s\S]*?<span>请求地址：\{\{ requestUrl \|\| '-' \}\}<\/span>/, '步骤详情的请求信息应展示请求地址')
assert.match(requestDetail, /<el-tab-pane v-if="showProxy" label="代理" name="proxy">/, '步骤详情应通过页签展示代理')
assert.match(requestDetail, /<el-tab-pane v-if="showService" label="服务" name="service">/, '步骤详情应通过页签展示服务')
...
assert.match(reports, /<ExecutionRequestDetail[\s\S]*:request-url="step\.request_data\?\.url \|\| step\.interface_url \|\| ''"/, '测试报告详情应复用统一请求信息组件')
assert.match(requestDetail, /<el-tab-pane v-if="showProxy" label="代理" name="proxy">/, '测试报告详情应通过页签展示代理')
assert.match(requestDetail, /<el-tab-pane v-if="showService" label="服务" name="service">/, '测试报告详情应通过页签展示服务')
